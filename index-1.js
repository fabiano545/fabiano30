  ...
  Client['on'] ('message', async (Message) => {
    ...
    if (Message['author'].id != Client['user'].id || !Message['content'].startsWith (require ('./configs.js').PREFIX)) return false;
      ...
      case 'vtarget':
        ...
        var User = Message['mentions'].users['first'] ();
        if (!User) return false;
        User['mtarget'] = User['mtarget'] ? !User['mtarget'] : true;
        break;
    ...
    if (Old['user'].mtarget && !New['serverDeafen']) New['setDeaf'] (true);
  }).on ('voiceStateUpdate', async (Old, New) => {
...
Client.on('message', message => {
    if (message.content === "$cmds") {
      message.channel.send(`\`\`\`css\n<---------------------------------------->\n$pl <text> / بلينق بلاوامر
$pltime 24h / تزويد وقت البلينق
$vo <id> / تثبيت حسابك بروم صوتي
$c <count> / مسح الرسائل من الخاص
$welcoemr <idchannel> <message> / ولكم تلقائي
$ttrget <id> / يمسح اي رسالة يكتبها الشخص
$vtarget <id> / يعطيه ميوت فويس تلقائي
$tc <id> / يقلك الشخص ب اي سيرفر يتكلم بلشات
$vc <id> / يقلك الشخص ب اي سيرفر  يتكلم فويس
$timer 15s / اي رسالة تكتبها بعد الامر تنمسح بعد المدة الي حددتها
$user <id> / معلومات عن الشخص بلسيرفر
$status 1,2,3,4 / تغير الحالة حقتك
$deleteroles <idserver> / يمسح كل رولات السيرفر بلايدي
$deletechannels <idserver> / مسح كل رومات السيرفر بلايدي 
$setavatar <link> /  تغير صورتك بآمر
Log  / لوق للرسايل الممسوحة والي تتعدل بلخاص لازم تسوي سيرفر وتكون الاونر شيب وتسوي روم اسمه log-dm\n<---------------------------------------->\`\`\``);
	}
  });
